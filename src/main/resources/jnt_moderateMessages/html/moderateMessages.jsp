...
<template:addResources type="javascript"
	resources="jquery.min.js,jquery-ui.min.js,jquery.blockUI.js,workInProgress.js,admin-bootstrap.js" />
              	...
              	<c:if test="${jcr:hasPermission(forumPost.parent.parent, 'moderatePost') and jcr:isNodeType(forumPost, 'jmix:moderated') and not forumPost.properties.moderated.boolean}">
              		<tr>
              			...
              			<td><span class="author"> <c:if
              					...
              					</c:if> <c:if test="${createdBy.string eq 'guest'}">${fn:escapeXml(forumPost.properties.pseudo.string)}</c:if>
            ...
            <h3><fmt:message key="siteSettings.label.moderateCommentMessages"/></h3>
                ...
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <c:forEach items="${CommentPostlist}" var="commentPost" varStatus="status">
                      ...
                      <c:if test="${jcr:isNodeType(commentPost, 'jnt:post')}">
                      	...
                      	<c:if test="${jcr:hasPermission(commentPost, 'deletePost')}">
                      		<tr>
                      			...
                      			<td><span class="author"> <c:if
                      						...
                      						<a
                      							href="<c:url value='${url.base}${functions:lookupUser(createdBy.string).localPath}.html'/>">${createdBy.string}</a>
                      					</c:if> <c:if test="${createdBy.string eq 'guest'}">${fn:escapeXml(commentPost.properties.pseudo.string)}</c:if>
                      					<c:if test="${createdBy.string ne ' guest '}">
                      						<jcr:node var="userNode"
                      							path="${functions:lookupUser(createdBy.string).localPath}" />
                      						<c:forEach items="${userNode.properties}" var="property">
                    ...
                    <c:choose>
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <c:forEach items="${blogPostlist}" var="blogPost" varStatus="status">
                              ...
                              <c:if test="${jcr:isNodeType(blogPost, 'jnt:post')}">
                              	...
                              	<c:if test="${jcr:hasPermission(blogPost, 'deletePost')}">
                              		<tr>
                              			...
                              			<td><span class="author"> <c:if
                              						...
                              						<a
                              							href="<c:url value='${url.base}${functions:lookupUser(createdBy.string).localPath}.html'/>">${createdBy.string}</a>
                              					</c:if> <c:if test="${createdBy.string eq 'guest'}">${fn:escapeXml(blogPost.properties.pseudo.string)}</c:if>
                              					<c:if test="${createdBy.string ne ' guest '}">
                              						<jcr:node var="userNode"
                              							path="${functions:lookupUser(createdBy.string).localPath}" />
                              						<c:forEach items="${userNode.properties}" var="property">
